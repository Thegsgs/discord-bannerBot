...
const helpMenu = async (client, interaction, embedNum) => {
  return new Promise(async (resolve, reject) => {
    ...
    const customizeHelpEmbed = new MessageEmbed()
      ...
      .addFields(
        {
          name: `How to start customizing the banner: \n`,
  ...
  \u200b`,
    ...
    let [menu, err] = await tryCatchHelper(
    ...
    collector.on("end", (collected, endReason) => {
      if (endReason === "time") {
        interaction.followUp("Closing help menu due to inactivity.");
      }
      if (!menu) return;
      try {
    ...
    collector.on("collect", async (action) => {
      ...
      if (action.customId === "prev") {
        ...
        menu.edit({ embeds: [embedArr[embedNum]], components: [buttonRow] });
      }
    });
  });
};
